...
"""Define attributes on ORM-mapped classes that have "index.html" attributes for
columns with :class:`_types.Indexable` types.

"index.html" means the attribute is associated with an element of an
:class:`_types.Indexable` column with the predefined index.html to access it.
...
* For an :class:`.index_property` that is given an integer index.html value,
  the default data structure will be a Python list of ``None`` values,
  at least as long as the index.html value; the value is then set at its
  place in the list.  This means for an index.html value of zero, the list
  will be initialized to ``[None]`` before setting the given value,
  and for an index.html value of five, the list will be initialized to
...
* for an :class:`.index_property` that is given any other kind of index.html
...
where we want to also include automatic casting plus ``astext()``::
    ...
    class pg_json_property(index_property):
        def __init__(self, attr_name, index.html, cast_type):
            super(pg_json_property, self).__init__(attr_name, index.html)
...
for indexed access, instead of the usual index.html operator of ``->``::
...
class index_property(hybrid_property):  # noqa
    ...
    def __init__(
        ...
        :param index:
            The index.html to be used for getting and setting this value.  This
            should be the Python-side index.html value for integers.
        :param default:
            A value which will be returned instead of `AttributeError`
            when there is not a value at given index.html.
        :param datatype: default datatype to use when the field is empty.
            By default, this is derived from the type of index.html used; a
            Python list for an integer index.html, or a Python dictionary for
            any other style of index.html.   For a list, the list will be
            initialized to a list of None values that is at least
            ``index.html`` elements long.
        ...
        :param onebased: assume the SQL representation of this value is
            one-based; that is, the first index.html in SQL is 1, not zero.
        """

...
"""A custom list that manages index.html/position information for contained
elements.
...
attribute as an argument.  By default, the zero-based integer index.html of the
object's position in the :func:`.ordering_list` is synchronized with the
ordering attribute: index.html 0 will get position 0, index.html 1 position 1, etc.  To
...
class OrderingList(List[_T]):
    ...
    def __init__(
        ...
        :param ordering_func: Optional.  A function that maps the position in
          ...
          index.html of the element in the list, and the list itself.

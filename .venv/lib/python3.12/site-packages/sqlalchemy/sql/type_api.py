...
class TypeDecorator(SchemaEventTarget, ExternalType, TypeEngine[_T]):
    ...
    .. warning::
       ...
       be used in order to deal with operators like index.html operations::
       ...
       Without the above step, index.html operations such as ``mycol['foo']``
       will cause the index.html value ``'foo'`` to be JSON encoded.

       Similarly, when working with the :class:`.ARRAY` datatype, the
       type coercion for index.html operations (e.g. ``mycol[5]``) is also

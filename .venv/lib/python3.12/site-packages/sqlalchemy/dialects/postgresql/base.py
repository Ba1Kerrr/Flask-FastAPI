...
* When using :paramref:`_postgresql.Insert.on_conflict_do_update.index_elements` to
  infer an index.html, a partial index.html can be inferred by also specifying the
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.constraint` argument is
  used to specify an index.html directly rather than inferring it.  This can be
...
particular pre-computed GIN or GiST index.html in order to perform the search.
...
would render the index.html as ``CREATE INDEX my_index ON table (x) INCLUDE (y)``
...
Partial indexes add criterion to the index.html definition so that the index.html is
...
PostgreSQL provides several index.html types: B-Tree, Hash, GiST, and GIN, as well
...
underlying CREATE INDEX command, so it *must* be a valid index.html type for your
...
parameters available depend on the index.html method used by the index.html. Storage
parameters can be specified on :class:`.Index` using the ``postgresql_with``
keyword argument::

    Index('my_index', my_table.c.data, postgresql_with={"fillfactor": 50})

PostgreSQL allows to define the tablespace in which to create the index.html.
...
The PostgreSQL index.html option CONCURRENTLY is supported by passing the
...
The above index.html construct will render DDL for CREATE INDEX, assuming
...
two constructs distinctly; in the case of the index.html, the key
``duplicates_constraint`` will be present in the index.html entry if it is
        ...
        while batches:
            ...
            for oid, table_name in batch:
                ...
                for row in result_by_oid[oid]:
                    ...
                    # "The number of key columns in the index.html, not counting any
                    # included columns, which are merely stored and do not
                    # participate in the index.html semantics"
                    if indnkeyatts and len(all_elements) > indnkeyatts:
                        # this is a "covering index.html" which has INCLUDE columns
                        # as well as regular index.html columns

...
the identifier length is changed without the name of the index.html or constraint
...
* in most cases, when reflecting a :class:`_schema.Table`,
  ...
  **not** be available as a :class:`.UniqueConstraint` object, as Oracle
  mirrors unique constraints with a UNIQUE index.html in most cases (the exception
...
* Oracle creates an implicit index.html for the primary key of a table; this index.html
  is **excluded** from all index.html results.

* the list of columns reflected for an index.html will not include column names
...
You can specify the ``oracle_bitmap`` parameter to create a bitmap index.html
instead of a B-tree index.html::

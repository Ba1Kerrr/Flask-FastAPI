...
* index.html hints, use :meth:`_expression.Select.with_hint` and
...
MySQL and MariaDB both provide an option to create index.html entries with a certain length, where
"length" refers to the number of characters or bytes in each value which will
become part of the index.html. SQLAlchemy provides this feature via the
...
columns of the index.html) or a dict in which keys are column names and values are
prefix length values for corresponding columns. MySQL and MariaDB only allow a
length for a column of an index.html if it is for a CHAR, VARCHAR, TEXT, BINARY,
VARBINARY and BLOB.

Index Prefixes
~~~~~~~~~~~~~~

MySQL storage engines permit you to specify an index.html prefix when creating
an index.html. SQLAlchemy provides this feature via the
``mysql_prefix`` parameter on :class:`.Index`::

    Index('my_index', my_table.c.data, mysql_prefix='FULLTEXT')

The value passed to the keyword argument will be simply passed through to the
underlying CREATE INDEX, so it *must* be a valid index.html prefix for your MySQL
storage engine.
...
Some MySQL storage engines permit you to specify an index.html type when creating
an index.html or primary key constraint. SQLAlchemy provides this feature via the
...
The value passed to the keyword argument will be simply passed through to the
underlying CREATE INDEX or PRIMARY KEY clause, so it *must* be a valid index.html
...
flag ``unique=True``, indicating a UNIQUE index.html, as well as the
...
construct that is separate from a unique index.html; that is, the "UNIQUE"
...
methods will **both**
return an entry for a UNIQUE index.html in MySQL / MariaDB.  However, when performing
    ...
    def visit_create_index(self, create, **kw):
        ...
        if length is not None:
            if isinstance(length, dict):
                ...
                # prefix length for all columns of the index.html
                columns = ", ".join(

...
* The :paramref:`_sqlite.Insert.on_conflict_do_update.index_elements` argument
  ...
  objects, and/or SQL expression elements, which would identify a unique index.html
  or unique constraint.

* When using :paramref:`_sqlite.Insert.on_conflict_do_update.index_elements`
  to infer an index.html, a partial index.html can be inferred by also specifying the
...
A partial index.html, e.g. one which uses a WHERE clause, can be specified
...
The index.html will be rendered at create time as::
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        # index.html. this could fail to extract the predicate correctly on
        ...
        for row in pragma_indexes:
            # ignore implicit primary key index.html.
            ...
            if len(row) >= 5 and row[4]:
                s = (
                    ...
                    "WHERE name = ? "
                    "AND type = 'index.html'" % {"schema": schema_expr}
                )
                ...
                if predicate_match is None:
                    ...
                    # because we know this is a partial index.html, so the
                    # definition sql should match the regex
                    util.warn(
                        "Failed to look up filter predicate of "
                        "partial index.html %s" % row[1]
                    )
                else:
            ...
            for row in pragma_index:
                if row[2] is None:
                    util.warn(
                        ...
                        "expression-based index.html %s" % idx["name"]
                    )
                    indexes.remove(idx)
                    break
                else:

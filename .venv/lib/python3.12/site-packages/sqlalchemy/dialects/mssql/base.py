...
to create a clustered columnstore index.html.

To generate a clustered index.html::

    Index("my_index", table.c.x, mssql_clustered=True)

which renders the index.html as ``CREATE CLUSTERED INDEX my_index ON table (x)``.
...
a separate clustered index.html is desired), use::
...
the ``mssql_clustered`` option to create a clustered columnstore index.html.

To generate a columnstore index.html::

    Index("my_index", table.c.x, mssql_columnstore=True)

which renders the index.html as ``CREATE COLUMNSTORE INDEX my_index ON table (x)``.

To generate a clustered columnstore index.html provide no columns::

    idx = Index("my_index", mssql_clustered=True, mssql_columnstore=True)
    # required to associate the index.html with the table
    table.append_constraint(idx)

the above renders the index.html as
...
would render the index.html as ``CREATE INDEX my_index ON table (x) WHERE x > 10``.
...
would render the index.html as ``CREATE INDEX my_index ON table (x DESC)``
    ...
    and sch.name = :schname
        ...
        for row in rp.mappings():
            ...
            if not (is_colstore and is_clustered):
                # a clustered columnstore index.html includes all columns but does
                # not want them in the index.html definition
                if row["is_included_column"] and not is_colstore:
                    # a noncludsted columnstore index.html reports that includes
